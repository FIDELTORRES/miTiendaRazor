...
namespace miTienda.Pages.Web
{
    public class IndexModel : PageModel
    {
        ...
        public async Task OnGetAsync(int? categoriaId, string busqueda)
        {
            ...
            {
                // Si falla la carga de categorías, continuar con lista vacía
                System.Console.WriteLine($"Error cargando categorías: {ex.Message}");
                CategoriasNivel1 = new List<CategoriaNivel1>();
            }
            ...
            try
            {
                // ✅ VERIFICAR QUE _context NO SEA NULL
                if (_context == null)
                {
                    ProductosPaginados = new PagedResult<ProductoTienda>
                    {
                        Items = new List<ProductoTienda>(),
                        TotalItems = 0,
                ...
                // ✅ ORDENAMIENTO (manejar null)
                query = query.OrderBy(pt => pt.Producto != null ? pt.Producto.Nombre : "");
                ...
                // 🔥 LOG PARA DEPURACIÓN - Mostrar cuántos productos encuentra
                System.Console.WriteLine($"=== DEPURACIÓN ===");
                System.Console.WriteLine($"Total productos en BD con Estado=1 y IdTienda=2: {totalItems}");
                System.Console.WriteLine($"Productos en esta página: {items.Count}");
                System.Console.WriteLine($"PageNumber: {PageNumber}");

                // ✅ ASIGNAR RESULTADO
                ProductosPaginados = new PagedResult<ProductoTienda>
                {
                    Items = items ?? new List<ProductoTienda>(),
            ...
            catch (System.Exception ex)
            {
                ...
                System.Console.WriteLine($"Error cargando productos: {ex.Message}");
                ...
                // ✅ OPCIONAL: Guardar error en TempData para mostrarlo en la vista
